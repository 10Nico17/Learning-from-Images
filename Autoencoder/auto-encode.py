...
class Autoencoder(nn.Module):
    ...
    def __init__(self, n_pixels, feature_length):
        ...
        self.activation = nn.ReLU()  # Aktivierungsfunktion hinzufügen
...
if __name__ == '__main__':
    ...
    feature_out = 150
    model = Autoencoder(n_pixels=x * y, feature_length=feature_out)
    # Define Loss
    ...
    if len(images_test) != 0 and len(reconstructed_images) != 0:
        ...
        plt.suptitle("Output features: "+str(feature_out))  # Hier wird der Titel hinzugefügt


        plt.savefig("auto_encoder_solution_"+str(feature_out)+"_output_features"+".png")
        plt.show()
    else:
        print(
